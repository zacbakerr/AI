...
def brute_force2(board, placedWords):
    ...
    #     if (len(csListAndPossibleWords[0][1])) == 1: csListAndPossibleWords.remove(csListAndPossibleWords[0])
    # print("")
    # for i in range(len(csListAndPossibleWords)):
    #     print(csListAndPossibleWords[0][0])
    # print("")
    # for i in range(len(csListAndPossibleWords)):
    # remove the ones with length 1
    minIndex = 0
    minVal = len(csListAndPossibleWords[0][1])
    for i in range(len(csListAndPossibleWords)):
        if len(csListAndPossibleWords[i][1]) < minVal:
            minVal = len(csListAndPossibleWords[i][1])
            minIndex = i
    csListAndPossibleWords = [csListAndPossibleWords[minIndex]]

    # check if the list of words is empty
    if len(csListAndPossibleWords[0][1]) == 0: return ""

    if csListAndPossibleWords[0][0][1] == "h":
        for word in csListAndPossibleWords[0][1]:
            if word in placedWords: continue
            bomb = False
            for j in range(len(word)):
                if augy[csListAndPossibleWords[0][0][0]+j] != "-" and augy[csListAndPossibleWords[0][0][0]+j] != word[j]: bomb = True
            if bomb: continue
            verdCounter = 0
            for j in range(len(word)):
                iIndexCounter = 0
                blockAI = csListAndPossibleWords[0][0][0]+j
                while blockAI > 0:
                    if augy[blockAI] == "#": break
                    blockAI -= (WIDTH+2)
                    iIndexCounter += 1
                blockAI += (WIDTH+2)
                iIndexCounter -= 1
                blockBI = csListAndPossibleWords[0][0][0]+j
            ...
            if verdCounter == len(word):
                if augy[csListAndPossibleWords[0][0][0]-1] != "#": continue
                istoavoid = []
                for j in range(len(word)):
                    if augy[csListAndPossibleWords[0][0][0]+j] != "-": istoavoid.append((csListAndPossibleWords[0][0][0]+j, augy[csListAndPossibleWords[0][0][0]+j]))
                augy = augy[:csListAndPossibleWords[0][0][0]] + word + augy[csListAndPossibleWords[0][0][0]+len(word):]
                ...
                augy = augy[:csListAndPossibleWords[0][0][0]] + "-"*len(word) + augy[csListAndPossibleWords[0][0][0]+len(word):]
                for j in range(len(istoavoid)):
                    augy = augy[:istoavoid[j][0]] + istoavoid[j][1] + augy[istoavoid[j][0]+1:]
    if csListAndPossibleWords[0][0][1] == "v":
        for word in csListAndPossibleWords[0][1]:
            if word in placedWords: continue
            bomb = False
            for j in range(len(word)):
                if augy[csListAndPossibleWords[0][0][0]+j*(WIDTH+2)] != "-" and augy[csListAndPossibleWords[0][0][0]+j*(WIDTH+2)] != word[j]: bomb = True
            if bomb: continue
            verdCounter = 0
            for j in range(len(word)):
                blockLI = csListAndPossibleWords[0][0][0]+j*(WIDTH+2)
                iIndexCounter = 0
                while blockLI > 0:
                ...
                blockLI += 1
                iIndexCounter -= 1
                blockRI = csListAndPossibleWords[0][0][0]+j*(WIDTH+2)
                while blockRI < len(augy):
                    if augy[blockRI] == "#": break
                    blockRI += 1
                horizTempWord = augy[blockLI:blockRI]
                # place correct letter of word in the correct place
                horizTempWord = horizTempWord[:iIndexCounter] + word[j] + horizTempWord[iIndexCounter+1:]
                ...
                break
            if verdCounter == len(word):
                if augy[csListAndPossibleWords[0][0][0]-(WIDTH+2)] != "#" and csListAndPossibleWords[0][0][0]-(WIDTH+2) >= 0: continue
                augy = augy[:csListAndPossibleWords[0][0][0]] + word[0] + augy[csListAndPossibleWords[0][0][0]+1:]
                for j in range(1,len(word)):
                    augy = augy[:csListAndPossibleWords[0][0][0]+j*(WIDTH+2)] + word[j] + augy[csListAndPossibleWords[0][0][0]+j*(WIDTH+2)+1:]
                ...
                augy = augy[:csListAndPossibleWords[0][0][0]] + "-" + augy[csListAndPossibleWords[0][0][0]+1:]
                for j in range(1,len(word)):
                    augy = augy[:csListAndPossibleWords[0][0][0]+j*(WIDTH+2)] + "-" + augy[csListAndPossibleWords[0][0][0]+j*(WIDTH+2)+1:]
    return ""
